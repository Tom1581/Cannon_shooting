...
function App() {
  ...
  const updatePhysics = (timestamp, currentFirePower, gameOver, currentLevel) => {
    ...
    const ENEMY_SPEED = 2.5 + (currentLevel * 0.5); 
    ...
    // 4. Enemy Spawning
    if (timestamp - state.lastEnemySpawn > Math.max(200, 1500 - (currentLevel * 250))) {
      ...
      const spawnGiant = () => {
        ...
        if (canSpawn) {
          let hp = isBoss ? (15 + currentLevel * 20) * 8 : (15 + currentLevel * 20);
          let radius = isBoss ? GIANT_RADIUS * 1.8 : GIANT_RADIUS;
          state.giants.push({
            x: width * 0.1 + Math.random() * width * 0.8, y: -50,
            hp: hp, maxHp: hp, vx: 0, vy: ENEMY_SPEED * (isBoss ? 0.3 : 0.6),
            isBoss: isBoss,
            radius: radius
          });
        }
  ...
  const render = (ctx, currentFirePower, timestamp) => {
    ...
    state.giants.forEach(g => {
      ...
      ctx.fillStyle = g.isBoss ? '#b33939' : '#ff5252'; 
